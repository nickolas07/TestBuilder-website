...
from skripteKontrollen.skripte.funktionen import *
from skripteKontrollen.skripte.plotten import *
...
def kurvendiskussion_polynom_parameter_2(nr, teilaufg=['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j']):
    ...
    if 'g' in teilaufg:
        ...
        if Rational(3,(faktor_1 + faktor_2 + faktor_3)) > 0:
            abhaengigkeit = r'\mathrm{mit~x \in \mathbb{R} ~und~ x > 0}'
        else:
            abhaengigkeit = r'\mathrm{mit~x \in \mathbb{R} ~und~ x < 0}'
        ...
        loesung.append(str(liste_teilaufg[i]) + r') \quad x ~=~' + (gzahl(xwert_wp_bruch)) + r'a \quad \vert \div'
                       ...
                       + gzahl(Rational(3,(faktor_1 + faktor_2 + faktor_3))) + r'x \quad' + abhaengigkeit
