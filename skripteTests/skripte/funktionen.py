...
def timer(func):
    ...
    def wrapper(*args, **kwargs):  # Erklärung eines Dekorators -> https://t1p.de/lqn4d
        start_time = time.perf_counter()  # Zeit vor dem Ausführen nehmen
        result = func(*args, **kwargs)  # Aufruf der eigentlichen Funktion mit ihren Argumenten
        end_time = time.perf_counter()  # Zeit nach dem Ausführen
        execution_time = end_time - start_time  # Vergangene Zeit berechnen

        print(f'\033[38;2;0;220;120m\033[1m"{func.__name__}" wurde in {round(execution_time, 3)} Sekunden '
              f'ausgeführt\033[0m')
        return result
    return wrapper
...
def packages(doc: Document):
    """
    Fügt unsere benutzten Pakete an das jeweilige Dokument an.
    """
...
def vektor_kuerzen(vec, p=50):
    ...
    for _ in vec:
        ...
        if liste[i] % 1 == 0:
            ...
            while (liste[i] * faktor[k]) % 1 != 0 and k+1 < p:
                k += 1
            liste = liste * faktor[k]
            i += 1
    # print('erweitert: ' + str(liste))
    teiler = [x + 1 for x in range(int(max(liste)/2), -1, -1)]
    for zahl in teiler:
        treffer = [1 for x in liste if x % zahl == 0]
        if sum(treffer) == len(vec):
            liste = liste / zahl
    # print('gekürzt: ' + str(liste))
    liste = np.array([int(element) if element % 1 == 0 else element for element in liste])
...
def ergebnisraum_zmZ(anzahl_ziehen, farbe1='weiß', farbe2='schwarz'):
    omega = [[farbe1 for _ in range(anzahl_ziehen)]]
    for anzahl in omega:
        i = 0
        for _ in anzahl:
            tubel = anzahl.copy()
...
def ergebnisraum_zoZ(az, anz_1, anz_2, farbe1='weiß', farbe2='schwarz'):
    ...
    if az > anz_ges:
        az = anz_ges
    omega1 = [[farbe1 for _ in range(az)]]
    if az > anz_1:
        omega = []
    else:
        omega = [[farbe1 for _ in range(az)]]
